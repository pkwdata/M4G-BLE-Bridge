...
static const char *TAG = "USB_SOFT_HOST";
...
static void hid_transfer_callback(usb_transfer_t *transfer)
{
    ...
    if (transfer->status == USB_TRANSFER_STATUS_COMPLETED) {
        if (ENABLE_DEBUG_LOGGING) {
            ...
            // Log the first few bytes of the HID report
            if (transfer->actual_num_bytes > 0) {
                ESP_LOG_BUFFER_HEX(TAG, transfer->data_buffer,
                                   MIN(transfer->actual_num_bytes, 8));
            }
        }

        // Convert CharaChorder report to standard HID keyboard report
        if (transfer->actual_num_bytes >= 4) {
            // CharaChorder report format: [report_id, modifier, reserved, key, ...]
            // Standard HID format: [modifier, reserved, key1, key2, key3, key4, key5, key6]
            uint8_t hid_report[8] = {0};

            if (transfer->data_buffer[0] == 0x01) { // Keyboard report
                hid_report[0] = transfer->data_buffer[1]; // Modifier byte
                hid_report[1] = 0; // Reserved byte
                hid_report[2] = transfer->data_buffer[3]; // First key
                // Additional keys could be added from positions 4+ if needed

                if (ENABLE_DEBUG_LOGGING) {
                    ESP_LOGI(TAG, "Converted HID report: mod=0x%02x key=0x%02x",
                             hid_report[0], hid_report[2]);
                    ESP_LOG_BUFFER_HEX(TAG, hid_report, 8);
                }

                // Forward the converted report to BLE
                static uint32_t last_report_time = 0;
                uint32_t current_time = xTaskGetTickCount();
                if (current_time - last_report_time >= pdMS_TO_TICKS(10)) { // Limit to max 100 reports/sec
                    send_hid_report_to_ble(hid_report, 8);
                    last_report_time = current_time;
                }
            }
        }

        // TODO: Parse and merge reports from both keyboard halves
    ...
    // Resubmit the transfer to continue receiving HID reports
    // Add a small delay to prevent CPU blocking
    ...
    if (err != ESP_OK) {
        ...
        // Don't continuously retry failed transfers - could cause infinite loop
...
void app_main(void)
{
    ...
    while (1) {
        ...
        if (active_hid_devices == 0) {
            no_devices_counter++;
            if (no_devices_counter % 10 == 0) { // Every 10 seconds
        ...
        if (encryption_completed && !ble_hid_notify_enabled && ble_conn_handle > 0) {
            ...
            if (elapsed_time >= 3000) { // 3 seconds
